...
def parse_args():
    ...
    parser.add_argument("--img-dir", default="E:/postdam/test/img")
    parser.add_argument("--mask-dir", default="E:/postdam/test/mask")
    parser.add_argument("--output-img-dir", default="E:/postdam_split/test1/img")
    parser.add_argument("--output-mask-dir", default="E:/postdam_split/test1/mask")
...
def pv2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 3, axis=0)] = [0, 255, 0]#綠
    mask_rgb[np.all(mask_convert == 0, axis=0)] = [255, 255, 255]#白
    mask_rgb[np.all(mask_convert == 1, axis=0)] = [255, 0, 0]#紅
    mask_rgb[np.all(mask_convert == 2, axis=0)] = [255, 255, 0]#黃
    mask_rgb[np.all(mask_convert == 4, axis=0)] = [0, 204, 255]#湖藍
    mask_rgb[np.all(mask_convert == 5, axis=0)] = [0, 0, 255]#藍
    return mask_rgb
...
def patch_format(inp):
   ...
   # m = sorted(glob.glob(os.path.join(self.kk, '*.pt')), key=os.path.getmtime)
    img_filename = os.path.basename(img_path)

    mask_filename = os.path.basename(mask_path)
    # print('1',img_filename)
    # print('2',mask_filename)
    # print('3',img_path)
    if eroded:
        mask_path1 =  mask_path + '_label_noBoundary.tif'
    else:
        mask_path1 =  mask_path + '_label.tif'
    if rgb_image:
        img_path1 = img_path+ '_RGB.tif'
    else:
        img_path1 = img_path+ '_IRRG.tif'
    img = Image.open(img_path1).convert('RGB')
    # print(img)
    mask = Image.open(mask_path1).convert('RGB')
    if gt:
...
if __name__ == "__main__":
    ...
    inp = [(img_path, mask_path, imgs_output_dir, masks_output_dir, False, gt, rgb_image,
